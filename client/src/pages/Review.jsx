...
const Review = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5124/api/review/batch', {
        orderId,
        reviews: reviews.map(r => ({ bookId: r.bookId, rating: r.rating, review: r.review }))
      }, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-4 bg-gradient-to-br from-purple-50 to-blue-100 min-h-screen rounded-xl shadow-2xl relative">
      <h1 className="text-3xl font-extrabold mb-8 text-center text-blue-800 drop-shadow-lg">Leave a Review</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {reviews.map((r, idx) => (
          <div key={r.bookId || idx} className="bg-white rounded-lg shadow p-6 flex flex-col gap-2">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
              <span className="font-semibold text-lg text-purple-700">{r.bookTitle}</span>
              <StarRating rating={r.rating} onChange={rating => handleRatingChange(idx, rating)} />
            </div>
            <textarea
              className="w-full border border-purple-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-300 text-gray-700 min-h-[80px] resize-none"
        ...
        {!success ? (
          <button
            type="submit"
            className="w-full py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-full font-bold text-lg shadow hover:from-blue-700 hover:to-purple-700 transition-colors duration-200 disabled:opacity-60"
            disabled={submitting}
          >
      ...
      {success && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-10 flex flex-col items-center animate-fade-in">
            ...
            <div className="text-2xl font-bold text-green-700 mb-2">Review Submitted!</div>
